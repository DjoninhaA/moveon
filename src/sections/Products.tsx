...
export function Products() {
  ...
  const next = useCallback(() => {
    setSlide((s) => (s + 1) % currentImages.length);
            ...
            <div className="relative rounded-2xl overflow-hidden shadow-2xl border border-gray-200/60 bg-gray-50">
              ...
              <div className="relative overflow-hidden bg-gray-50" style={{ minHeight: "340px" }}>
                {currentImages.map((img, i) => (
                  <div
                    key={i}
                    className={`transition-opacity duration-500 ${i === slide ? "opacity-100" : "opacity-0 absolute inset-0 pointer-events-none"}`}
                  >
                    <Image
                      src={img.src}
                      alt={img.label}
                ...
                {currentImages.map((_, i) => (
                  <button
                    ...
                    className={`rounded-full transition-all duration-300 ${
                      i === slide ? "w-6 h-2 bg-primary" : "w-2 h-2 bg-gray-300 hover:bg-gray-400"
                    }`}
